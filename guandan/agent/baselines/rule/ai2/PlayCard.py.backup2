...
class PlayCard():
    ...
    def FreePlay(self, handCards, curRank, fullActionList = None):
        print("Free play handCards:", handCards, "   restHandsCount:", self.stragegy.restHandsCount)
        handValue, handActions = CountValue().HandCardsValue(handCards, 0, curRank)
        #print(handActions)
        #Strategy.SetBeginning(0)
        self.stragegy.SetRole(handValue, handActions, curRank)
        self.stragegy.makeReviseValues()
        ...
        #beginning
        bestPlay = {}
        if (len(handCards)>=15 or self.stragegy.roundStage != 'ending'):
            minValue = 100
            for action in handActions:
                actionValue = CountValue().ActionValue(action, action['type'], action['rank'], curRank) - self.stragegy.freeActionRV[action['type']] \
                        - self.stragegy.freeActionRV[(action['type'],action['rank'])]
        ...
        #print(Strategy.freeActionRV[('Pair','Q')])
            ...
            for i in range(0, len(config.cardTypes)):
                ...
                for rank1 in actionList[type]:
                    for card in actionList[type][rank1]:
                        ...
                        thisHandValue += self.stragegy.freeActionRV[type]
                        if (type, rank) in self.stragegy.freeActionRV.keys():
                            thisHandValue += self.stragegy.freeActionRV[(type, rank)]
            ...
            for action in additionalActionList:
                ...
                restValue, restActions = CountValue().HandCardsValue(restCards, 0, curRank)
                restValue += self.stragegy.handRV[type]
    ...
    def RestrictedPlay(self, handCards, formerAction, curRank, fullActionList = None):
        print("Restricted Play handCards:", handCards,"   restHandsCount:", self.stragegy.restHandsCount)
        actionList = CreateActionList().CreateList(handCards)
        ...
        self.stragegy.SetRole(maxValue, restActions, curRank)
        self.stragegy.makeReviseValues()
        maxValue += self.stragegy.restrictedActionRV["PASS"]
            ...
            for rank1 in actionList[type]:
                for card in actionList[type][rank1]:
                    ...
                    if (CompareRank().Larger(type, rank, card, formerAction, curRank)):
                        ...
                        thisHandValue = CountValue().ActionValue(action, type, rank, curRank)
                        thisHandValue += self.stragegy.restrictedActionRV[type]
                        if (type, rank) in self.stragegy.restrictedActionRV.keys():
                            thisHandValue += self.stragegy.restrictedActionRV[(type, rank)]
                        #print(Strategy.actionValueRevise)
                        #print(rank, card, thisHandValue, restValue)
                        if (thisHandValue < 0): thisHandValue = 0
                        if (thisHandValue + restValue > maxValue or (thisHandValue + restValue == maxValue and \
